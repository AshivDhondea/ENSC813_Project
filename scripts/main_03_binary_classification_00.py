# -*- coding: utf-8 -*-
"""
Created on Sun Apr 12 01:38:11 2020
...
10-fold cross-validation for benchmarking methods

Ref: The CNN used is the one which Chollet used for Cats vs Dogs in his book
...
random_state_chosen = 42; # We use the same seed in this set of simulations to 
# ensure we are testing on the same test set.
...
# x_trainval and y_trainval are used for training the CNN and evaluating it.
...
# --------------------------------------------------------------------------- #
## Data generator for learning, defining parameters for learning
...
for train, val in kfold.split(x_trainval,y_trainval):
    ...
    # --------------------------------------------------------------------------- #
    history_of_model_1 = model_1.fit_generator(datagen.flow(x_trainval[train], y_trainval[train], batch_size=batch_size_chosen),
    ...
    test_results[:,fold_no-1] = model_1.evaluate(x_test, y_test, batch_size=batch_size_chosen);
...
# --------------------------------------------------------------------------- #
# Save the results for benchmark comparison later on.
