# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 02:04:31 2020
...
# First Layer --------------------------------------------------------------  # 
model_1 = Sequential()
model_1.add(Conv2D(32, (3, 3), input_shape=(image_dim, image_dim, 3)))
...
# ---- classification ------------------------------------------------------- #
model_1.add(Flatten())  # this produces a 1D feature vector
model_1.add(Dense(64))
model_1.add(Activation('relu'))
model_1.add(Dropout(0.5))
model_1.add(Dense(output_nodes))
...
model_1.compile(loss='binary_crossentropy',optimizer=opt,metrics=['categorical_accuracy']);
...
history_of_model_1 = model_1.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size_chosen),
...
model_1.save(full_name+'_model.h5')
model_1.save_weights(full_name+'_weights.h5')
print('Model saved\n')
...
test_results = model_1.evaluate(x_test, y_test, batch_size=batch_size_chosen);
print('test loss, test acc:', test_results);

y_pred_test_continuous =  model_1.predict(x_test,batch_size= batch_size_chosen);
...
y_pred_train_continuous =  model_1.predict(x_train,batch_size= batch_size_chosen);
...
plt.title(r"\textbf{Confusion matrix on test set with Model 1}",fontsize=12);
...
val_loss_min_acc = max(history_of_model_1.history['categorical_accuracy'][val_loss_min_arg],history_of_model_1.history['val_categorical_accuracy'][val_loss_min_arg])
...
plt.plot(history_of_model_1.history['categorical_accuracy'],color='mediumblue')
plt.plot(history_of_model_1.history['val_categorical_accuracy'],color='forestgreen')
...
plt.annotate(r"\textbf{Validation accuracy = %.3f}" %history_of_model_1.history['val_categorical_accuracy'][val_loss_min_arg],(0,0.25) );
