# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 02:43:24 2020
...
# First Layer --------------------------------------------------------------  # 
model_3 = Sequential()
model_3.add(Conv2D(32, (3, 3), input_shape=(image_dim, image_dim, 3)))
...
# ---- classification ------------------------------------------------------- #
model_3.add(Flatten())  # this produces a 1D feature vector
model_3.add(Dense(256))
model_3.add(Activation('relu'))
model_3.add(Dropout(0.5))
model_3.add(Dense(output_nodes))
...
model_3.compile(loss='binary_crossentropy',optimizer=opt,metrics=['categorical_accuracy']);
...
history_of_model_3 = model_3.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size_chosen),
...
model_3.save(full_name+'_model.h5')
model_3.save_weights(full_name+'_weights.h5')
print('Model saved\n')

# Get the dictionary containing each metric and the loss for each epoch
history_dict = history_of_model_3.history
## Save it under the form of a json file
json.dump(history_dict, open(full_name+'_history.json', 'w'));

# Evaluate trained model on the test set                                       
test_results = model_3.evaluate(x_test, y_test, batch_size=batch_size_chosen);
print('test loss, test acc:', test_results);

y_pred_test_continuous =  model_3.predict(x_test,batch_size= batch_size_chosen);
...
y_pred_train_continuous =  model_3.predict(x_train,batch_size= batch_size_chosen);
...
plt.title(r"\textbf{Confusion matrix on test set with Model 3}",fontsize=12);
sn.heatmap(df_cm, annot=True, annot_kws={"size": 16}) # font size
plt.savefig(full_name+'_confusion_matrix.pdf');
plt.show()
# --------------------------------------------------------------------------- #
# Plot results
val_loss_min_arg = np.argmin(history_of_model_3.history['val_loss']);
val_loss_min = min(history_of_model_3.history['val_loss']);
val_loss_min_acc = max(history_of_model_3.history['categorical_accuracy'][val_loss_min_arg],history_of_model_3.history['val_categorical_accuracy'][val_loss_min_arg])
...
plt.plot(history_of_model_3.history['categorical_accuracy'],color='mediumblue')
plt.plot(history_of_model_3.history['val_categorical_accuracy'],color='forestgreen')
...
plt.annotate(r"\textbf{Validation accuracy = %.3f}" %history_of_model_3.history['val_categorical_accuracy'][val_loss_min_arg],(0,0.25) );
...
plt.plot(history_of_model_3.history['loss'],color='mediumblue')
plt.plot(history_of_model_3.history['val_loss'],color='forestgreen')
