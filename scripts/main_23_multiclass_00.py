# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 02:51:36 2020
...
fig = plt.figure(1);
plt.title(r"\textbf{Split: training set (%.3f), validation set (%.3f) \& test set (%.3f)}" %(train_size,val_size,test_size) ,fontsize=12)
bar_width = 0.24*4/len(y_train_count_dic);
opacity = 0.8
n_groups = 3;
index = np.arange(n_groups);


color_list = ['seagreen','cornflowerblue','lightcoral','gold','mediumorchid','maroon','limegreen'];

chosen_classes = ['Audi','BMW','Lexus','Mercedes-Benz'];

for k in range(len(y_train_count_dic)):    
    rects1 = plt.bar(index + (1.0*bar_width)*(k),np.array([y_train_count_dic[k],y_val_count_dic[k],y_test_count_dic[k]]),bar_width,color=color_list[k],alpha=opacity,label=r'%s' %chosen_classes[k])

plt.xticks(index + (len(y_train_count_dic)-1)*bar_width/2, (r'Training', r'Validation',r'Test'))
...
# First Layer --------------------------------------------------------------  # 
model_4 = Sequential()
model_4.add(Conv2D(32, (3, 3), input_shape=(image_dim, image_dim, 3)))
...
# ---- classification ------------------------------------------------------- #
model_4.add(Flatten())  # this produces a 1D feature vector
model_4.add(Dense(256))
model_4.add(Activation('relu'))
model_4.add(Dropout(0.3))
model_4.add(Dense(output_nodes))
...
model_4.compile(loss='binary_crossentropy',optimizer=opt,metrics=['categorical_accuracy']);
...
datagen = ImageDataGenerator(rescale=1./255,rotation_range=0,width_shift_range=0.0,height_shift_range=0.0,shear_range=0.0,zoom_range=0.0,horizontal_flip=True,fill_mode='reflect') 
...
history_of_model_4 = model_4.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size_chosen),
...
model_4.save(full_name+'_model.h5')
model_4.save_weights(full_name+'_weights.h5')
print('Model saved\n')

# Get the dictionary containing each metric and the loss for each epoch
history_dict = history_of_model_4.history
## Save it under the form of a json file
json.dump(history_dict, open(full_name+'_history.json', 'w'));

# Evaluate trained model on the test set                                       
test_results = model_4.evaluate(x_test, y_test, batch_size=batch_size_chosen);
print('test loss, test acc:', test_results);

y_pred_test_continuous =  model_4.predict(x_test,batch_size= batch_size_chosen);
...
y_pred_train_continuous =  model_4.predict(x_train,batch_size= batch_size_chosen);
...
plt.title(r"\textbf{Confusion matrix on test set with Model 4}",fontsize=12);
sn.heatmap(df_cm, annot=True, annot_kws={"size": 16}) # font size
plt.savefig(full_name+'_confusion_matrix.pdf');
plt.show()
# --------------------------------------------------------------------------- #
# Plot results
val_loss_min_arg = np.argmin(history_of_model_4.history['val_loss']);
val_loss_min = min(history_of_model_4.history['val_loss']);
val_loss_min_acc = max(history_of_model_4.history['categorical_accuracy'][val_loss_min_arg],history_of_model_4.history['val_categorical_accuracy'][val_loss_min_arg])
...
plt.plot(history_of_model_4.history['categorical_accuracy'],color='mediumblue')
plt.plot(history_of_model_4.history['val_categorical_accuracy'],color='forestgreen')
plt.plot([val_loss_min_arg,val_loss_min_arg],[0,val_loss_min_acc],color='maroon',linestyle='dashed');

ax.set_xlabel(r'Epoch')
ax.set_ylabel(r'Accuracy'); 
xticks_range=np.arange(0,num_epochs,20,dtype=np.int64);
...
plt.annotate(r"\textbf{Validation accuracy = %.3f}" %history_of_model_4.history['val_categorical_accuracy'][val_loss_min_arg],(0,0.25) );
...
plt.plot(history_of_model_4.history['loss'],color='mediumblue')
plt.plot(history_of_model_4.history['val_loss'],color='forestgreen')
