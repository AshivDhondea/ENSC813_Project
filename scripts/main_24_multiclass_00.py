...
# First Layer --------------------------------------------------------------  # 
model_5 = Sequential()
model_5.add(Conv2D(32, (3, 3), input_shape=(image_dim, image_dim, 3)))
...
model_5.add(Conv2D(512, (3, 3)))
model_5.add(Activation('relu'))
model_5.add(MaxPooling2D(pool_size=(2, 2)))


# ---- classification ------------------------------------------------------- #
model_5.add(Flatten())  # this produces a 1D feature vector
model_5.add(Dense(256))
model_5.add(Activation('relu'))
model_5.add(Dropout(0.3))
model_5.add(Dense(output_nodes))
...
model_5.compile(loss='binary_crossentropy',optimizer=opt,metrics=['categorical_accuracy']);
...
callbacks_early_stopping = [EarlyStopping(monitor='val_loss', patience=50,restore_best_weights=True)];

history_of_model_5 = model_5.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size_chosen),
...
model_5.save(full_name+'_model.h5')
model_5.save_weights(full_name+'_weights.h5')
print('Model saved\n')

# Get the dictionary containing each metric and the loss for each epoch
history_dict = history_of_model_5.history
## Save it under the form of a json file
json.dump(history_dict, open(full_name+'_history.json', 'w'));

# Evaluate trained model on the test set                                       
test_results = model_5.evaluate(x_test, y_test, batch_size=batch_size_chosen);
print('test loss, test acc:', test_results);

y_pred_test_continuous =  model_5.predict(x_test,batch_size= batch_size_chosen);
...
y_pred_train_continuous =  model_5.predict(x_train,batch_size= batch_size_chosen);
...
# --------------------------------------------------------------------------- #
# Plot results
val_loss_min_arg = np.argmin(history_of_model_5.history['val_loss']);
val_loss_min = min(history_of_model_5.history['val_loss']);
val_loss_min_acc = max(history_of_model_5.history['categorical_accuracy'][val_loss_min_arg],history_of_model_5.history['val_categorical_accuracy'][val_loss_min_arg])
...
plt.plot(history_of_model_5.history['categorical_accuracy'],color='mediumblue')
plt.plot(history_of_model_5.history['val_categorical_accuracy'],color='forestgreen')
...
plt.annotate(r"\textbf{Validation accuracy = %.3f}" %history_of_model_5.history['val_categorical_accuracy'][val_loss_min_arg],(0,0.25) );
...
plt.plot(history_of_model_5.history['loss'],color='mediumblue')
plt.plot(history_of_model_5.history['val_loss'],color='forestgreen')
